...
namespace ShapeGame
{
    ...
    void RenderSystem_::Update(float DeltaTime)
    {
        ...
        auto view = registry.view<SegmentComponent, TransformComponent>();
        view.each(
            [this](const auto& entity, const SegmentComponent& segment, const TransformComponent& trans)
            {
                std::vector<Vector2> vertices;
                ...
                GetVertices(vertices, segment, thickness, trans.pos, trans.rot, hasThickness, has_endcap);
                if (hasThickness)
                {
