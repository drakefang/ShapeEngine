...
void RenderSystem::GetVertices(std::vector<Vector2> &vertices,
                               const SegmentComponent &segment, float thickness,
                               const Vector2 &loc, float rot, bool hasThickness,
                               bool hasEndcap) {
  ...
  if (hasEndcap) { // ===== 端点圆形（Triangle Fan） =====
...
void RenderSystem::Update(float DeltaTime) {
  ...
  auto view =
      registry.view<SegmentComponent, LocationComponent, RotationComponent>();
  view.each([this](const auto &entity, SegmentComponent segment,
                   LocationComponent loc, RotationComponent rot) {
    std::vector<Vector2> vertices;
    bool hasThickness = registry.any_of<ThicknessComponent>(entity);
    bool has_endcap = registry.any_of<EndCapComponent>(entity);
    float thickness = 1.f;
    if (hasThickness) {
      thickness = registry.get<ThicknessComponent>(entity).thickness;
    }
    Color c = BEIGE;
    ...
    GetVertices(vertices, segment, thickness, loc.pos, rot.rot, hasThickness,
                has_endcap);
    if (hasThickness) {
